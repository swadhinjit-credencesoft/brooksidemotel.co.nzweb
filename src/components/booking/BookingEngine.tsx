...
function roomHighlights(rid: string) {
  const slug = STAAH_SLUGS[rid];
  if (slug) { const d = getRoom(slug); if (d) return d.highlights.slice(0, 4); }
  return [];
}
...
type View = "results" | "guests" | "payment" | "confirm";
...
export default function BookingEngine() {
  ...
  const onSelectRoom = useCallback(async (q: RoomQuote) => {
    ...
    } catch { /* continue without detailed rates */ }
    finally { setRateLoading(false); setView("guests"); }
  ...
  const perDay = rateDetail?.perDay ?? null;
  ...
  return (
    <div className="be-wrap">
      ...
      <div className="be-steps">
        <span className={`be-step${view === "results" || view === "guests" || view === "payment" || view === "confirm" ? " be-step--active" : ""}`}>
      ...
      {view === "guests" && selected && (
        <div className="be-guests-view">
          <button type="button" className="be-back" onClick={() => { setView("results"); setRateDetail(null); }}>
